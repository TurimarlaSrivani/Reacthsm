...
class PatientHistory extends Component {
  ...
  //Calling response for getAllPatientHistory
  componentDidMount() {
  ...
  render() {
    //sorting patientHistory by patientHistoryId
    const { search, sortColumn, patient_history } = this.state;
    var sorted = [];
    if (search) {
      sorted = patient_history.filter(
        (history) => history.recordedDate == search
      );
    } else {
      sorted = _.orderBy(
    ...
    return (
      //form for patientHistory to search,add,update, delete
        ...
        <table className="table mt-3 shadow">
          <thead className="table-dark">
            <tr>
              ...
              <th onClick={() => this.handleSort("recordedDate")}>
                Recorded Date
              </th>
              ...
              <tr key={history.patientHistoryId}>
                ...
                <td>
                  <Link to={`/history/${history.patientHistoryId}`}>
                    {/* Button to update the patientHistory  */}
                    <button className="btn btn-secondary">Update</button>
                  </Link>
                  {/* Button to delete the patientHistory  */}
                  <button
                    className="btn btn-danger ml-2"
                    onClick={() =>
                      this.deletePatientHistory(history.patientHistoryId)
                    }
                  >
                    Delete
                  </button>
